...
- The retired root `android/` Bubblewrap wrapper: React Native now owns the Android application. Its Git history remains available for migration archaeology.
...
- Under `mobile/`, `npm audit --omit=dev` and `npx expo install --check` must pass. Run `npm run test:tooling`, `npm run lint`, `npm run typecheck`, `npm test -- --runInBand`, `npm run doctor`, and `npm run export` twice.
...
- The retired TWA and native Android app share `zone.syrian.app`. A production replacement must use the existing Play app-signing identity and a `versionCode` above the highest Play Console upload. The generated local Release uses debug signing and must not be uploaded. Keep `public/.well-known/assetlinks.json` until installed TWA clients are retired, then update it only with the production native signing certificate if HTTPS App Links are enabled.
...
Two exact-context postinstall patches protect the pinned MapLibre iOS dependency. One disables its unsafe native warning callback before a percent-encoded glyph URL can be treated as a format string. The other resets GeoJSON, image, raster DEM, raster, and vector source wrappers when Fabric recycles their component views. Both scripts are idempotent and fail when upstream source drifts, and `npm run test:tooling` verifies the installed dependency. Revisit and remove a patch only after the upstream package contains the same lifecycle or logging fix.

The app bundles exact dark and light vector map styles. Mishwar and Transit select the active theme, fetch CARTO vector tile metadata, MVT tiles, and sprites, and resolve IBM Plex Sans Arabic glyphs against Laravel. The server contains all 256 fixed BMP ranges for both font stacks used by those styles. `scripts/build-map-glyphs.test.mjs` locks the 512 PBF files to one aggregate digest and verifies every server and bundled `text-font` consumer. The bundled styles preserve the source country-label override for Palestine. Transit and Mishwar display OpenStreetMap and CARTO attribution. Population and Syrian identity maps use bundled geometry on a local background. Remote base tiles aren't part of the offline promise. If they're unavailable, the selected background, bundled boundaries, cached routes and stops, and an offline notice remain visible.
