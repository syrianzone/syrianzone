...
class ExternalDataController extends Controller
{
    ...
    public function govapps()
    {
        ...
        $data = Cache::remember('external_govapps_data', 600, function () use ($url) {
            $data = [];
            try {
                $response = Http::get($url);
                if ($response->successful()) {
                    ...
                    Log::warning('Failed to fetch external govapps data: HTTP ' . $response->status());
                }
            } catch (\Exception $e) {
                Log::warning('Failed to fetch external govapps data', [
                    'controller' => static::class,
                    'error'      => $e->getMessage(),
                ]);
            }
