...
export default function TransitAdminPage() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('transit_admin_token')
    setToken(saved)
  ...
  // If the saved token is rejected by the server, clear it and show login
  useEffect(() => {
    if (draftsError && String(draftsError).includes('UNAUTHORIZED')) {
      localStorage.removeItem('transit_admin_token')
      setToken(null)
    }
  }, [draftsError])
