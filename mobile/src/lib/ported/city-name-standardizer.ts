...
export function getCanonicalCityName(cityName: string): string {
    ...
    const direct = CANONICAL_NAMES[cityName];
    if (direct) {
        return direct;
    }

    const normalized = normalizeForMatching(cityName);
    const matched = CANONICAL_NAMES[normalized];
    if (matched) {
        return matched;
    }
...
export function standardizeCityNames(cities: { [key: string]: number }): { [key: string]: number } {
    ...
    Object.entries(cities).forEach(([cityName, value]) => {
        ...
        standardized[canonical] = (standardized[canonical] ?? 0) + value;
    });
...
/*
PORT STATUS
  source:     resources/js/Lib/city-name-standardizer.ts (153 lines)
  confidence: high
  todos:      0
  notes:      Canonical aliases, aggregation, and governorate order remain platform independent.
*/
