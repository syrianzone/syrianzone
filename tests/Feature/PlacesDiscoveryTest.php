...
test('guides aggregates approved places per user with the exact shape', function () {
    ...
    Place::factory()->approved()->create(['user_id' => $guide->id, 'description' => 'قصير', 'saves_count' => 3]);
    Place::factory()->approved()->create(['user_id' => $guide->id, 'description' => 'قصير', 'saves_count' => 4]);
...
test('level boundaries: 14 is L1, exactly 15 is L2, 74 is L2, 75 is L3', function () {
    $levels = app(GuideLevelService::class);
    // pins the thresholds the frontend milestones table mirrors
    expect(GuideLevelService::LEVELS)->toBe([
    ...
    $l2 = discoveryUser();
    $l3 = discoveryUser();
    Place::factory()->approved()->create(['user_id' => $l2->id, 'description' => 'قصير', 'saves_count' => 59]); // 74
    Place::factory()->approved()->create(['user_id' => $l3->id, 'description' => 'قصير', 'saves_count' => 60]); // 75
...
test('guides points ignore pending and rejected places entirely', function () {
    $guide = discoveryUser();
    Place::factory()->approved()->create(['user_id' => $guide->id, 'description' => 'قصير', 'saves_count' => 1]);
...
test('guides points are computed inside the guides cache', function () {
    $guide = discoveryUser();
    $place = Place::factory()->approved()->create(['user_id' => $guide->id, 'description' => 'قصير', 'saves_count' => 0]);
